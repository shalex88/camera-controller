...
namespace camera_service::core {
    Core::Core(std::unique_ptr<data::ICameraHal> camera)
        ...
        if (!camera_) {
            throw std::invalid_argument("[Core] Cannot initialize Core with null camera");
        }
    }
    ...
    Core::~Core() {
        if (isInitialized()) {
            if (shutdown().isError()) {
                LOG_ERROR("[Core] Failed to shut down Core properly");
            }
        }
    }

    Result<void> Core::initialize() {
        LOG_INFO("[Core] Initializing Core...");
        ...
        LOG_INFO("[Core] Core initialized successfully.");
    ...
    Result<void> Core::shutdown() {
        ...
        LOG_INFO("[Core] Shutting down Core...");
        ...
        LOG_INFO("[Core] Core shut down successfully.");
        return Result<void>::success();
    }
    ...
    Result<void> Core::setZoom(const types::zoom zoom_level) {
        if (!isInitialized()) {
            return Result<void>::error("[Core] Core not initialized");
        }
    ...
    Result<types::zoom> Core::getZoom() const {
        if (!isInitialized()) {
            return Result<types::zoom>::error("[Core] Core not initialized");
        }
    ...
    Result<types::focus> Core::getFocus() const {
        if (!isInitialized()) {
            return Result<types::focus>::error("[Core] Core not initialized");
        }
