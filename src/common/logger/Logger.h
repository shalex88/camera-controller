...
namespace camera_service::common {
    ...
    public:
        LayerLogger(std::shared_ptr<LoggerInterface> logger, std::string layer_name,
                    const std::string& log_level = "info") : logger_impl_(std::move(logger)),
...
#define CONFIGURE_GLOBAL_LOGGER(name, level) do { \
    ...
    if (adapter) { \
        camera_service::common::GlobalLogger::getInstance().setLoggerAdapter(std::move(adapter)); \
        camera_service::common::GlobalLogger::getInstance().setLogLevel(level); \
    } \
} while(0)

#define SET_LOG_LEVEL(level) camera_service::common::GlobalLogger::getInstance().setLogLevel(level)

#define LOG_TRACE(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Trace, __VA_ARGS__)
#define LOG_DEBUG(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Debug, __VA_ARGS__)
#define LOG_INFO(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Info, __VA_ARGS__)
#define LOG_WARN(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Warn, __VA_ARGS__)
#define LOG_ERROR(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Error, __VA_ARGS__)
#define LOG_CRITICAL(...) camera_service::common::GlobalLogger::getInstance().log(LoggerInterface::LogLevel::Critical, __VA_ARGS__)
