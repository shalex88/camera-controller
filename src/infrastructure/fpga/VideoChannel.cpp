...
namespace {
    Result<void> configureChannel(const int channel_num) {
        ...
        {
            const auto global_reg = std::make_unique<service::infrastructure::RegisterImplUio>("/dev/uio12");
        ...
        {
            const auto mipi_reg = std::make_unique<service::infrastructure::RegisterImplUio>("/dev/uio14");
            if (const auto result = mipi_reg->set(0x800D0000, 0x00000000); result.isError()) {
                return Result<void>::error("Failed to disable MIPI RX");
            }
        }
        {
            const auto test_pattern_reg = std::make_unique<service::infrastructure::RegisterImplUio>("/dev/uio6");
        ...
        {
            const auto mux_reg = std::make_unique<service::infrastructure::RegisterImplUio>("/dev/uio7");
