#include "GenTLImpl.h"
#include "../genicam/FpgaTransport.h"
...
DeviceModule::DeviceModule(std::string id, std::string device)
    ...
    info_.vendor = "Adimec";
    info_.model = "TMX5";
    info_.serialNumber = "00000001";
    info_.version = "1.0.0";
    info_.displayName = "CoaXPress Camera";
    info_.userDefinedName = "NFOV Camera";
    info_.tlType = "CXP";
...
GC_ERROR DeviceModule::getInfo(DEVICE_INFO_CMD cmd, INFO_DATATYPE* type, void* buffer, size_t* size) {
    if (type) *type = INFO_DATATYPE_STRING; // String type by default
    ...
    switch (cmd) {
        case DEVICE_INFO_ID:
            return copyString(info_.id, buffer, size);
        case DEVICE_INFO_VENDOR:
            return copyString(info_.vendor, buffer, size);
        case DEVICE_INFO_MODEL:
            return copyString(info_.model, buffer, size);
        case DEVICE_INFO_TLTYPE:
            return copyString("CXP", buffer, size);
        case DEVICE_INFO_DISPLAYNAME:
            return copyString(info_.displayName, buffer, size);
        case DEVICE_INFO_ACCESS_STATUS:
            return copyInteger(static_cast<int32_t>(DEVICE_ACCESS_STATUS_READWRITE), buffer, size);
        case DEVICE_INFO_USER_DEFINED_NAME:
            return copyString(info_.userDefinedName, buffer, size);
        case DEVICE_INFO_SERIAL_NUMBER:
            return copyString(info_.serialNumber, buffer, size);
        case DEVICE_INFO_VERSION:
            return copyString(info_.version, buffer, size);
        case DEVICE_INFO_TIMESTAMP_FREQUENCY:
            return copyInteger(info_.timestampFrequency, buffer, size);
        default:
