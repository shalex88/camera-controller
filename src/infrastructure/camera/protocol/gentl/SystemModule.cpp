...
namespace {
    constexpr const char* TL_ID = "FpgaCXP";
    constexpr const char* TL_VENDOR = "CameraService";
    constexpr const char* TL_MODEL = "FPGA CoaXPress Transport Layer";
    constexpr const char* TL_VERSION = "1.0.0";
    constexpr const char* TL_NAME = "FpgaCXPProducer";
    constexpr const char* TL_PATHNAME = "/usr/lib/genicam/gentl/FpgaCXP.cti";
    constexpr const char* TL_DISPLAYNAME = "FPGA CoaXPress Producer";
    ...
    GC_ERROR copyIntegerInfo(T value, INFO_DATATYPE* type, void* buffer, size_t* size) {
        ...
        if (buffer == nullptr) {
            *size = required;
            if (type) {
                if (sizeof(T) == 2) *type = INFO_DATATYPE_UINT16;
                else if (sizeof(T) == 4) *type = INFO_DATATYPE_UINT32;
                else if (sizeof(T) == 8) *type = INFO_DATATYPE_UINT64;
                else *type = INFO_DATATYPE_UNKNOWN;
            }
            return GC_ERR_SUCCESS;
        }
...
SystemModule::SystemModule() {
    discoverInterfaces();
    LOG_INFO("GenTL System Module initialized");
}

SystemModule::~SystemModule() {
    LOG_INFO("GenTL System Module closed");
}
...
GC_ERROR SystemModule::getInfo(TL_INFO_CMD cmd, int32_t* type, void* buffer, size_t* size) {
    if (type) *type = 1; // String type by default
...
GC_ERROR SystemModule::getInterfaceInfo(const char* ifaceID, INTERFACE_INFO_CMD cmd, int32_t* type, void* buffer, size_t* size) {
    if (ifaceID == nullptr) {
        return GC_ERR_INVALID_PARAMETER;
    }
    ...
    auto it = std::find_if(interfaces_.begin(), interfaces_.end(),
        [ifaceID](const InterfaceInfo& info) { return info.id == ifaceID; });
...
GC_ERROR SystemModule::openInterface(const char* ifaceID, std::unique_ptr<InterfaceModule>& interface) {
    ...
    try {
        ...
        LOG_INFO("Opened interface: {}", it->id);
        return GC_ERR_SUCCESS;
...
//-----------------------------------------------------------------------------
InterfaceModule::InterfaceModule(std::string id) : id_(std::move(id)) {
    discoverDevices();
    LOG_INFO("GenTL Interface Module initialized: {}", id_);
}

InterfaceModule::~InterfaceModule() {
    LOG_INFO("GenTL Interface Module closed: {}", id_);
}
...
GC_ERROR InterfaceModule::getDeviceInfo(const char* devID, DEVICE_INFO_CMD cmd, int32_t* type, void* buffer, size_t* size) {
    if (devID == nullptr) {
        return GC_ERR_INVALID_PARAMETER;
    }
    ...
    auto it = std::find_if(devices_.begin(), devices_.end(),
        [devID](const DeviceInfo& info) { return info.id == devID; });
...
GC_ERROR InterfaceModule::openDevice(const char* devID, DEVICE_ACCESS_FLAGS accessMode, std::unique_ptr<DeviceModule>& device) {
    if (devID == nullptr) {
        return GC_ERR_INVALID_PARAMETER;
    }
    ...
    if (accessMode != DEVICE_ACCESS_READONLY && accessMode != DEVICE_ACCESS_CONTROL && accessMode != DEVICE_ACCESS_EXCLUSIVE) {
        return GC_ERR_ACCESS_DENIED;
    }

    try {
        device = std::make_unique<DeviceModule>(it->id, "/dev/mem");
        LOG_INFO("Opened device: {}", it->id);
        return GC_ERR_SUCCESS;
    } catch (const std::exception& e) {
